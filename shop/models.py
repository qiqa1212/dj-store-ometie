...
class Order(models.Model):
    ...
    status = models.CharField(
        max_length=32, choices=STATUS_CHOICES, default=STATUS_CART)
    amount = models.DecimalField(
        max_digits=20, decimal_places=2, blank=True, null=True)
    ...
    def get_cart(user: User):
        ...
        if not cart:
            cart = Order.objects.create(
                user=user, status=Order.STATUS_CART, amount=0,)
        return cart
    ...
    def get_amount_of_unpaid_orders(user: User):
        amount = Order.objects.filter(user=user, status=Order.STATUS_WAITING_FOR_PAYMENT).aggregate(
            Sum("amount"))["amount__sum"]
        return amount or Decimal(0)
